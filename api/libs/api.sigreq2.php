 ...
 * Base signup requests handling class
 When installing this as part of the module do not forget to update DarkVoid
 */
class SignupRequests2 {
    ...
    public function renderAjListData() {
        ...
        if (!empty($this->requests)) {
            foreach ($this->requests as $io => $eachreq) {
                ...
                $actlinks = wf_Link('?module=sigreq2&showreq=' . $eachreq['id'], wf_img('skins/icon_search_small.gif') . ' ' . __('Show'), true, '');
    ...
    public function renderList() {
        ...
        if (zb_CheckTableExists('sigreqconf')) {
            if (cfr('SIGREQCONF')) {
                $confControl = wf_Link('?module=sigreq2&settings=true', wf_img('skins/settings.png', __('Settings')), false) . ' ';
            } else {
                $confControl = '';
            }
        } else {
        ...
        $viewControl = wf_Link('?module=sigreq2&calendarview=true', wf_img('skins/icon_calendar.gif', __('As calendar')), false, '');
    ...
    public function renderCalendar() {
                ...
                $calendarData.="
                      {
                        ...
                        url: '?module=sigreq2&showreq=" . $each['id'] . "',
                        start: new Date(" . $date . "),
                        end: new Date(" . $date . "),
                       " . $coloring . "     
                   },
        ...
        $viewControl = wf_Link('?module=sigreq2', wf_img('skins/icon_table.png', __('Grid view')), false, '');
        show_window($confControl . __('Available signup requests') . ' ' . $viewControl, $result);
    }
    ...
    public function showRequest($reqid) {
		...
		$refName = zb_UserGetRealName(vf($reqdata['reflogin'],3));
		...
		//Refs
		$cells = wf_TableCell(__('Ref login'));
		$cells.= wf_TableCell("<a href='/index.php?module=userprofile&username=".$reqdata['reflogin']."'>".$reqdata['reflogin']."</a>");
		$rows.= wf_TableRow($cells, 'row3');
		$cells = wf_TableCell(__('Ref name'));
		$cells.= wf_TableCell("<a href='/index.php?module=userprofile&username=".$reqdata['reflogin']."'>".$refName."</a>");
		$rows.= wf_TableRow($cells,'row3');
        ...
        $actlinks = wf_BackLink('?module=sigreq2');

        if (cfr('SIGREQEDIT')) {
            if ($reqdata['state'] == 0) {
                $actlinks.=wf_Link('?module=sigreq2&reqdone=' . $reqid, wf_img_sized('skins/icon_active.gif', '', '10') . ' ' . __('Close'), false, 'ubButton');
            } else {
                $actlinks.=wf_Link('?module=sigreq2&requndone=' . $reqid, wf_img_sized('skins/icon_inactive.gif', '', '10') . ' ' . __('Open'), false, 'ubButton');
            }
        }

        if (cfr('SIGREQDELETE')) {
            $deletelink = ' ' . wf_JSAlert("?module=sigreq2&deletereq=" . $reqid, web_delete_icon(), $this->messages->getDeleteAlert());
        } else {
            $deletelink = '';
        }
     ...
     * @return int
     */
    public function getAllNewCount2() {
...
class SignupConfig2 {
    ...
    public function renderForm() {
        ...
        $result.= wf_BackLink('?module=sigreq2');
        return ($result);
    }
